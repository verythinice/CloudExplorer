	...
	function($, _, Backbone) {
		...
		var modalDialogView = Backbone.View.extend({
			...
			ensureModalContainer: function(target) {
				if (target != null) {
					// A target is passed in, we need to re-render the modal
					// container into the target.
			...
			ensureModalBlanket: function() {
				...
				if (this.modalBlanket.length == 0) {
					this.modalBlanket = $("<div id='modal-blanket'>").css({
						...
						top: 0,
						left: 0,
						height: $(document).height(), // Span the full
														// document height...
					...
					// Ensure the blanket spans the whole document, screen may
					// have been updated.
			...
			hideModal: function() {
				...
				if (this.options.bodyOverflowHidden === true) {
					$(document.body).css("overflow",
							this.originalBodyOverflowValue);
				}
	
				var container = this.modalContainer;
				$(this.modalContainer).fadeOut(this.options.fadeOutDuration,
					function() {
						container.remove();
					});
			},
			...
			getCoordinate: function(coordinate, css) {
				if (typeof (css[coordinate]) !== "undefined") {
					...
					delete css[coordinate]; // Don't apply positioning to the
											// $el, we apply it to the modal
			...
			// Re-centre the modal dialog after it has been
			// displayed. Useful if the height changes after initial
			// rendering eg; jquery ui tabs will hide tab sections
			recentre: function() {
				var $el = $(this.el);
				var coords = {
			...
			getOffsets: function() {
				...
				if (this.options.showModalAtScrollPosition) {
					offsetY = $(document).scrollTop(), offsetX = $(document)
							.scrollLeft()
				}
			...
			showModal: function(options) {
				...
				$(document.body).keyup(this.keyup); // This handler is unbound
													// in hideModal()
				this.modalBlanket.click(this.click); // This handler is
														// unbound in
														// hideModal()
	
				if (this.options.bodyOverflowHidden === true) {
					this.originalBodyOverflowValue = $(document.body).css(
							"overflow");
				...
				// Only apply default centre coordinates if no css positions
				// have been supplied
				if (coords.isEmpty()) {
					var positionY = centreY - ($el.outerHeight() / 2);
					if (positionY < 10)
					...
					// Overriding the coordinates with explicit values if they
					// are passed in
									...
									function() {
										$(this)
												.css(
														"background-image",
														"url("
																+ view.options.closeImageHoverUrl
																+ ") !important");
									},
									function() {
										$(this)
				...
				this.modalContainer.animate(animateProperties,
